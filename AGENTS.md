...
src/db/          schema.ts (all 25 tables) · seed.ts
src/lib/actions/     the ONLY write path (16 files)
src/lib/validation/  Zod schemas, one file per action domain
src/lib/{money,splits,balances,quests,trips,geo,places,music,telegram}/  pure domain logic + tests
